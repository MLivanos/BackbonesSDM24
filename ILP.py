...
class ModelExplanation():
	...
	def ILP_Solve(self):
		...
		for c in self.class_neurons.keys():
			explanations.append([0] * sum(self.architecture))
		for c in self.class_neurons.keys():
			e = ClassExplanation(c, self.class_neurons[c],
				self.test_neurons[c], self.weights, self.architecture,
				self.threshold, minsup_threshold = self.minsup_threshold)
			transactions.append(e.transactions)
		i = 0
		for transaction in transactions:
			transactions[i] = transaction.values.tolist()
			i += 1
		# MIP ILP
		m = Model(sense=MAXIMIZE)
		n = sum(self.architecture)
		explanations = [ [m.add_var(var_type=BINARY) for i in range(n)] for j in range(len(transactions)) ]
		# Instances
		gamma_1 = []
		for i in range(n):
			gamma_1.append(m.add_var(var_type=BINARY))
		# Neurons
		gamma_2 = []
		for j in range(len(transactions)):
			for k in range(len(transactions[j])):
				gamma_2.append(m.add_var(var_type=BINARY))
		# Diversity
		gamma_3 = []
		for i in range(n):
			gamma_3.append(m.add_var(var_type=BINARY))
		# Coverage
		counter = 0
		for j in range(len(explanations)):
			for k in range(len(transactions[j])):
				for i in range(n):
					m += gamma_1[i] + gamma_2[counter] + explanations[j][i] - transactions[j][k][i] >= 0
				counter += 1
		# Diversity
		for j in range(len(explanations)):
			for k in range(len(explanations)):
				if j != k:
					for i in range(n):
						m += explanations[j][i] + explanations[k][i] <= 1 + gamma_3[i]
		# Connected
		for j in range(len(explanations)):
			end = 0
			begining = 0
			for k in range(len(self.architecture)):
				end += self.architecture[k]
				m += xsum(explanations[j][begining:end]) >= 1
				begining = end
		# Gamma
		m += xsum(gamma_1[j] for j in range(n)) <= len(gamma_1) * 0.28
		m += xsum(gamma_2[j] for j in range(len(transactions) * len(transactions[j]))) <= len(gamma_2) * 0.0
		m += xsum(gamma_3[j] for j in range(n)) <= len(gamma_3) * 0.55
		# Maximize
		m.objective = xsum(explanations[j][i] for i in range(n) for j in range(len(self.architecture)))
		# Optimize
		m.optimize()
		selected = ([[i for i in range(sum(self.architecture)) if explanations[j][i].x >= 0.99] for j in range(len(explanations))])
		input(selected)
